...
I_ext(1:Ne,1)=0.0;    % input to the E population
% Mg
Mg=1;          % mM, Mg concentration for NMDA synapses for the norm [Jahr and Stevens, 1990]
% CL
Clo_E=130;        % mM                                
Vhalf_E=40;                % KCC2 1/2
HCO3o_E=26;       % mM
...
load('Net_transition_1_Mg1_Iext.mat');
load('Net_burst_extKo_init_40.mat');

IE_ext(1:end)=0.4;      % muA/cm^2 external input current to the E population 0.35
II_ext(1:end)=0.4;
...
Tr=15;        % threshould for amplitude peak =35 for seizure detection
...
%{
if Q<16
PATH=ceil(((Q-1)*2)^2/841*100); % <100% pathology
else
    PATH=100;                   % 100% pathology
end
%}
...
% Npath IS TAKEN FROM THE FILE

%      Npath=randperm(Ne,ceil(PATH/100*Ne));               % pecrcentage KCC2(-) cells     
%      Npath=central_elements(sqrt(Ne),(Q*2));       % PATHOLOGY IN A CENTRAL CLUSTER
%      Npath=[];                                     % NO PATHOLOGY

% ANOTHER KCC2 LEVEL !!!
       Ikcc2_E=2*ones(Ne,1);   % KCC2 max current 1.5 norm, 0 path       
       Nnorm=1:1:Ne;
       Nnorm(intersect(Npath,Nnorm))=[];  
       Ikcc2_E(Npath)=0;            % PATHOLOGY      
...
%% ICs E POPULATION
 ...
 Cli_E=subplus(random('normal',5,het_E*6.37,Ne,1));              % mM 5.13, Ikcc2=0.5
 Ko_E=subplus(random('normal',3.5,0,Ne,1));                     % mM
 ...
 Ko_E(Npath)=subplus(random('normal',3.5,het_E*3.46,length(Npath),1));            % mM
...
% ION CONCENTRATIONS %
 Ko_E=(kK_E/F/d_E*(IK_E +2*Ikpump_E -Ikcc2_E.*(VK_E-VCL_E)./((VK_E-VCL_E)+Vhalf_E)) +Glia_E +D_E./dx_E^2.*(sum(Ko_E(l),2)-4*Ko_E) +D_E_slice./dx_ext_E^2.*(Ko_bath-Ko_E) )*dt + Ko_E;
 ...
 % input to the population, external input, rec. E&I + noise + external input
 IE=IE_ext +gEE_AMPA.*(V_AMPA-VD_E) +(gEE_NMDA.*(V_NMDA-VD_E))./(1+Mg/3.57*exp(-0.062*VD_E)) +gIE_GABA.*(VGABA_E-VD_E) +IE_AMPA;
...
%% Check if there is seizure, put cells back to norm

if t*dt>=sw    
    sw=sw+dsw;              % go to the next sweep
    [freq,psdx,Hz,p_amp] = spect_peak(LFP,dt/1000,50);
         
    if  p_amp>=Tr                     % if peak amplitude > Tr - criteria for seizure
        Ikcc2_E(Npath)=2;             % put the KCC2 cells back to norm
    end
...
parsave(sprintf('Net_burst_extKo_IN_path_%d.mat',PATH),PATH,Npath,Nnorm,time,SIMULATION_TIME,dt,Ne,Ni,firings_E,firings_I,Konorm,Kopath,Ko_E,Clinorm,Clipath,Cli_E,VEnorm,VI1,VEpath,LFP);           % save *.mat

%savefig(sprintf('Net_burst_extKo_IN_path_%d.fig',PATH));           % safe *.fig

%saveas(gcf,sprintf('Net_burst_extKo_IN_path_%d.jpg',PATH),'jpg');  % save *.jpg
